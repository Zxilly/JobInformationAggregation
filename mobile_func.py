...
def get_work(conv, course_info):
    ...
    for one_course in course_info:
        # print(one)
        # print(url + '?courseId=' + str(one_course['course_id']) + '&classId=' + str(one_course['class_id']))
        ...
        # html_bs = BeautifulSoup(str(html_obj.content.decode('utf-8')), 'lxml')
        #print(html_bs.prettify())
        # exit(0)
        work_list = html_bs.find_all('li')
        for one_task in work_list:
            if (one_task.find('span').string == '未交') and (one_task.find('img')['src']==r'/images/work/phone/task-work.png'):
                # print(one_task)
                ...
                work_info.append(
                    {'work_name': work_name, 'course_name': course_name, 'left_time': left_time, 'work_url': work_url})
