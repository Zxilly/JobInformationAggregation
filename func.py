...
def login(conv):
    code_html_url = 'https://passport2.chaoxing.com/cloudscanlogin?mobiletip=JIA%e6%8e%88%e6%9d%83%e8%af%b7%e6%b1%82&pcrefer=http://i.chaoxing.com'
    code_html = conv.get(code_html_url,headers=headers)
    code_html_obj = BeautifulSoup(code_html.content, "html.lxml")
...
def get_work_info(conv, info_list):
    ...
    def is_li_but_has_no_lookLi(tag):
        # print(type(tag))
    ...
    for work_one_list in info_list:  # 开始处理一门课
        ...
        print(ul_obj)
        print("\n\n\n\n")
        undo_work_one_info_list = ul_obj.find_all(is_li_but_has_no_lookLi)
...
def parse_work(work_list):
    for one_work in work_list:
        ...
        print('\n\n\n\n')
        #work_status = work_tag.find('strong').string.strip()
        #print(work_status)
